...
//product routes
app.post('/product/add-category', verifyToken, upload.any(), (req, res) => {
...
app.get('/product/get-category/:id?', verifyToken, (req, res) => {
    axios.get(`${SERVICES.product}/get-category/${req.params.id || ''}`)
        .then(response => res.status(200).json(response.data))
        .catch(error => handleError(error, res));
});

app.post('/product/add-product', verifyToken, upload.any(), (req, res) => {
    const formData = createFormData(req);
...
app.get('/product/get-product/:id?', verifyToken, (req, res) => {
    axios.get(`${SERVICES.product}/get-product/${req.params.id || ''}`)
        .then(response => res.status(200).json(response.data))
        .catch(error => handleError(error, res));
});

app.get('/product/get-product-by-category/:id?', verifyToken, (req, res) => {
    axios.get(`${SERVICES.product}/get-product-by-category/${req.params.id || ''}`)
        .then(response => res.status(200).json(response.data))
        .catch(error => handleError(error, res));
});

app.get('/product/get-brand/:id?', verifyToken, (req, res) => {
    axios.get(`${SERVICES.product}/get-brand/${req.params.id || ''}`)
        .then(response => res.status(200).json(response.data))
        .catch(error => handleError(error, res));
});

app.post('/product/add-brand', verifyToken, (req, res) => {
    const formData = createFormData(req);
...
app.get('/user/:id?', verifyToken, (req, res) => {
    axios.get(`${SERVICES.product}/:id?/${req.params.id || ''}`)
        .then(response => res.status(200).json(response.data))
        .catch(error => handleError(error, res));
});
...
function handleError(error, res) {
    ...
    if (error.response) {
        // request made, but server responded with an error
